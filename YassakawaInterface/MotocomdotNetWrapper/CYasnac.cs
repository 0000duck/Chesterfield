...
namespace MotocomdotNetWrapper
{
    ...
    public class CYasnac
    {
        ...
        public int GetAlarm(out CErrorData error, out ArrayList alarmlist)
        {
            short ret;
            short errorno = -1;
        ...
        public short MovlJoint(StringBuilder moveSpeedSelection, double speed, short toolNo, ref double targetPosition)
        {
            lock (m_YasnacAccessLock)
            {
                return CMotocom.BscPMovl(m_Handle, moveSpeedSelection, speed , toolNo, ref targetPosition);
            }
        }

        public short MovjJoint(StringBuilder moveSpeedSelection, double speed, short toolNo, ref double targetPosition)
        {
            lock (m_YasnacAccessLock)
            {
                return CMotocom.BscPMovl(m_Handle, moveSpeedSelection, speed, toolNo, ref targetPosition);
